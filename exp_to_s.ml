...
type expr= Varint of int | Varfloat of float | Plus of expr | Moins of expr |Par of expr | Ent of expr | Flot of expr |
           ...
           Plusflot of expr*expr | Multflot of expr*expr | Sousflot of expr*expr;;
...
let exp_main expr=
    ...
    let rec auxmain expr= match expr with
        ...
        |Sousint(exp1,exp2)->(auxmain exp1) ++
                        pushq (reg rax) ++
                        (auxmain exp2) ++
                        popq (rcx) ++
        ...
        |Moins(exp1)-> (auxmain exp1) ++
                        subq (imm 0) (reg rax);
        |Varfloat(a)-> movsd (val1) (reg xmm0)++
                        l:= !l ++ inline "val"^string_of_int(!i)^" : .double"^a;
                        incr i;

        |Plusflot(exp1,exp2)-> auxmain exp1 ++
                                pushsd (reg xmm0) ++
                                auxmain exp2 ++
                                popsd (reg xmm1)++
                                addsd (reg xmm1) (reg xmm0);

        |Sousflot(exp1,exp2)-> auxmain exp1 ++
                                pushsd (reg xmm0) ++
                                auxmain exp2 ++
                                popsd (reg xmm1) ++
                                subsd (reg xmm1) (reg xmm0);
        |Multflot(exp1,exp2)-> auxmain exp1 ++
                                pushsd (reg xmm0) ++
                                auxmain exp2 ++
                                popsd (reg xmm1)++
                                imulsd (reg xmm1) (reg xmm0);
        |Divint(exp1,exp2)-> auxmain exp1 ++
                            pushq (reg rax) ++
                            auxmain exp2 ++
                            popq (reg rdi)
                            movq (reg rax) (reg rsi) ++
                            divq (reg rsi);
        |Mod(exp1,exp2)-> auxmain exp1 ++
                            pushq (reg rax) ++
                            auxmain exp2 ++
                            popq (reg rdi)
                            movq (reg rax) (reg rsi) ++
...
let code= {text=
            globl "main" ++ label "main" ++
             auxmain expr ++(if est_int then call print_int
             else call print_float)++ ret ++
             fin ++
             data=
    label "S_int" ++ string "%d" ++ !l;} in
             let c= open_out "print_exp.s" in
             let fmt = formatter_of_out_channel c in
             X86_64.print_program fmt code;
             close_out c;;
